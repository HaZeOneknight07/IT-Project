document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('searchInput').addEventListener('keypress', function(event) {
    if (event.key === 'Enter') {
      ...
      var searchTermsForNonManagers = ['login', 'siteinformation', 'siteinfo', 'report', 'hub', 'rates', 'documents', 'complete', 'insuredinfo', 'reportinfo', 'lv', 'xactimate', 'xact', 'x', 'sedgwick', 'srs', 's'];
      // List of search terms for managers
      var allowedSearchTermsForManagers = ['rates', 'lv', 'x', 'xact', 'xactimate', 'srs', 'salesforce', 'sedgwick', 's',];
      ...
      if (loggedInUser === 'managers@gelder.co.uk') {
         ...
         if (allowedSearchTermsForManagers.includes(searchTerm)) {
          ...
          // Handle unauthorized search terms for managers
